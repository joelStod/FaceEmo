%% For version 1.4 - Simulated Data

% Input parameters (in order)
% sigma, generalization, 1 to 15, mparams(1)
% theta, inverse temperature, 0 to 10, mparams(2)
% effEpsMax, max effective learning rate, 0 to 1, mparams(3)
% p, initial indifference point, 0 to 15 mparams(4)
% s, initial weight slope,  0 to 10 mparams(5)
% gA, guessing parameter for angry, 0 to 0.5 mparams(6)
% gH, guessing parameter for happy, 0 to 0.5 mparams(7)
% trainThresh, values <= the trainThresh are category 0 mparams(8)
...
% 3 each in block 1 and 12 each across blocks 1-6.
block = zeros(n,1);
stim = zeros(n,1);
...
% The model only cares about the category of the trial stimulus.
% Because feedback is deterministic, the model takes raw choice and
% reward and converts it to category. It then uses category to determine
% feedback. 

% Category is purely dependent on the category boundary, with
% stimuli > trainThresh having an angry response (1). So, if we set reward
% to be 1's then cho == category at the outset.
cho = zeros(n,1);
cho(stim > mparams(8)) = 1;
rew = ones(n,1);

% Generate choice probabilities according to the parameters
choiceprob = mod1_4(block,stim,cho,rew,mparams(1:7));
...
for t = 1:n
    ...
    if (cho(t) == 0 && stim(t) > mparams(8)) || ...
            (cho(t) == 1 && stim(t) <= mparams(8))
        rew(t) = 0;
    end
end
